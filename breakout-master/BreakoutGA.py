...
def ANN(weights, inputs):
    ...
    for j in range(len(finalOut)):
        ...
        finalOut[j] = activationSigmoid(x + (threshold * weights[j+5][5])) # Or some other activation function
...
while generation < 5000:
    ...
    #Every generation save the fittest chromosome, it's fitness, and the average fitness of the population
    if generation%1 == 0:
        fitnessFile = open("Fitnesses.txt", "a")
        fitnessFile.write(str(generation) + " " + str(calcFittest(population, fitnesses)) + " " + str(max(fitnesses)) + " " + str(totalFitness/n) + "\n")
        fitnessFile.close()
